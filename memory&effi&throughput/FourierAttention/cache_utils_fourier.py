...
from fourier_triton_valid4_v2 import MultiDimHiPPO2, mat_fourier_avg_std_triton, mat_avg_std_triton
@dataclass
class ExtraConfig:
    """
...
class DynamicCache(Cache):
    """
    An optimized dynamic cache for generative models, specifically tailored for high-performance GPUs like the H100.

    This cache pre-allocates memory to its maximum size to avoid costly runtime concatenations. It manages a
    circular buffer for the "local" window and uses a dedicated "middle" section for compressed representations.
    This design minimizes Python overhead and leverages PyTorch's performance features like `torch.compile`.

    Cache Structure:
    - Initial Tokens: A fixed number of tokens from the beginning of the sequence.
    - Middle Tokens (Compressed): A compressed representation of tokens between the initial and local windows.
    - Local Window: A circular buffer holding the most recent tokens.
    """

    
    def __init__(self, extra_config: ExtraConfig, num_hidden_layers: Optional[int] = None) -> None:
        super().__init__()
        
        # --- Configuration ---
        self.max_position_embeddings = extra_config.max_position_embeddings
        self.num_key_value_heads = extra_config.num_key_value_heads
        self.head_dim = extra_config.hidden_size // extra_config.num_attention_heads
        self.dtype = extra_config.dtype
        self.num_hidden_layers = extra_config.num_hidden_layers
        self.device = torch.device("cuda:0")

        # --- Eviction Strategy Config ---
        self.max_local_len = extra_config.maxlocallen
        self.max_mid_states = extra_config.maxmidstates
        self.num_init_tokens = extra_config.numinittokens
        self.total_max_len = self.num_init_tokens + self.max_mid_states + self.max_local_len

        # --- Cache Tensors (Pre-allocation) ---
        ...
        self.T = self.max_position_embeddings - self.num_init_tokens - self.max_local_len

        # --- Compression and Dimension Splitting ---
        self._load_critical_dims(extra_config)
        
        self.hippo_models = nn.ModuleList()
        for i in range(extra_config.num_hidden_layers):
            # Each layer gets its own HIPPO model instance if non-critical dims exist
            dt = 1.0 / self.T
            hippo = MultiDimHiPPO2(
                N=(self.num_key_value_heads * self.head_dim - self.k_non_critical_start[i]) * self.max_mid_states,
                input_dim=(self.num_key_value_heads * self.head_dim - self.k_non_critical_start[i]),
                method='legt', dt=dt, T=1
            ).to(self.device)
            self.hippo_models.append(hippo)

    def _load_critical_dims(self, extra_config: ExtraConfig):
        """Loads and prepares critical/non-critical dimension indices from a JSON file."""
        with open(extra_config.non_critical_dims_path, "r") as f:
            non_critical_dims_data = json.load(f)

        self.k_non_critical_start: List[int] = []
        self.v_non_critical_start: List[int] = []
        self.q_non_critical_start: List[int] = []
        self.num_non_compress_k_per_q_head: List[torch.Tensor] = []
        self.num_non_compress_v_per_q_head: List[torch.Tensor] = []

        num_layers = extra_config.num_hidden_layers

        for i in range(num_layers):
            self.k_non_critical_start.append(non_critical_dims_data[i]["num_k_non_compress"])
            self.v_non_critical_start.append(non_critical_dims_data[i]["num_v_non_compress"])
            self.q_non_critical_start.append(non_critical_dims_data[i]["num_q_non_compress"])
            self.num_non_compress_k_per_q_head.append(torch.tensor(non_critical_dims_data[i]["num_non_compress_k_per_q_head"], device=self.device, dtype=torch.int32))
            self.num_non_compress_v_per_q_head.append(torch.tensor(non_critical_dims_data[i]["num_non_compress_v_per_q_head"], device=self.device, dtype=torch.int32))
                
    def _initialize_cache_for_layer(self, bsz: int, layer_idx: int):
        raise NotImplementedError("Not implemented _initialize_cache_for_layer")
    ...
    def update(
        ...
        This optimized version uses pre-allocated buffers and index manipulation.

        Args:
            key_states (`torch.Tensor`): The new key states to cache, shape [bsz, num_heads, q_len, head_dim].
            value_states (`torch.Tensor`): The new value states to cache, shape [bsz, num_heads, q_len, head_dim].
            layer_idx (`int`): The layer index.
        """
        bsz, num_heads, q_len, head_dim = key_states.shape
        
        # Reshape to [bsz, q_len, -1] once
        key_states = key_states.transpose(1, 2).reshape(bsz, q_len, -1)
        value_states = value_states.transpose(1, 2).reshape(bsz, q_len, -1)
        
        # --- Eviction Logic ---
        # prefill only
        if q_len > 1: 
            # self._seen_tokens.append(q_len)
            # Number of tokens to evict from the local window into the middle compressed section
            num_to_evict = (q_len) - (self.num_init_tokens + self.max_local_len)
            # remain = num_to_evict % 256
            # num_to_evict -= remain
            # self.max_local_len += remain
            self._seen_tokens.append(q_len)
            
            # The indices to read from for eviction are in the circular buffer
            eviction_start_pos = self.num_init_tokens
            
            # --- Compression Step ---
            k_evict_nc = key_states[:, eviction_start_pos : eviction_start_pos + num_to_evict, self.k_non_critical_start[layer_idx]: ]
            k_evict_c = key_states[:, eviction_start_pos : eviction_start_pos + num_to_evict, :self.k_non_critical_start[layer_idx] ]
            k_evict_nc_compressed: torch.Tensor = self.hippo_models[layer_idx](base_input=k_evict_nc)
            # print("$$$ layer_idx: ", layer_idx)
            # print("$$$ k_evict_nc.shape: ", k_evict_nc.shape, k_evict_nc.dtype)
            # print("$$$ k_evict_c.shape: ", k_evict_c.shape, k_evict_c.dtype)
            # print("$$$ k_evict_nc_compressed.shape: ", k_evict_nc_compressed.shape, k_evict_nc_compressed.dtype)
            # print("$$$ ---")
            # $$$ layer_idx:  26
            # $$$ k_evict_nc.shape:  torch.Size([1, 60488, 512]) torch.float16
            # $$$ k_evict_c.shape:  torch.Size([1, 60488, 512]) torch.float16
            # $$$ k_evict_nc_compressed.shape:  torch.Size([1, 1024, 512]) torch.float32
            # u_avg, u_std = mat_avg_std_triton(k_evict_nc.contiguous())
            # i_avg, i_std = mat_fourier_avg_std_triton(k_evict_nc_compressed.contiguous(), k_evict_nc.size(1), T=self.T)
            # self.k_scale.append(u_std / i_std)
            # k_evict_nc_compressed = k_evict_nc_compressed * self.k_scale[-1]
            # k_bias_expand = torch.zeros(k_evict_nc_compressed.shape[0], key_states.shape[-1],\
            #     device=k_evict_nc_compressed.device, dtype=torch.float32)
            # k_bias_expand[:, :self.k_non_critical_start[layer_idx]] = u_avg - i_avg * self.k_scale[-1]
            # self.k_bias.append(k_bias_expand)

            k_evict_nc_compressed_expand = torch.zeros(k_evict_nc_compressed.shape[0], k_evict_nc_compressed.shape[1], key_states.shape[-1], \
                device=k_evict_nc_compressed.device, dtype=key_states.dtype)
            k_evict_nc_compressed_expand[:, :, self.k_non_critical_start[layer_idx]:] = k_evict_nc_compressed
            
            v_evict_nc = value_states[:, eviction_start_pos : eviction_start_pos + num_to_evict, self.v_non_critical_start[layer_idx]: ]
            v_evict_c = value_states[:, eviction_start_pos : eviction_start_pos + num_to_evict, :self.v_non_critical_start[layer_idx] ]
            v_evict_nc_compressed: torch.Tensor = self.hippo_models[layer_idx](base_input=v_evict_nc)
            # u_avg, u_std = mat_avg_std_triton(v_evict_nc.contiguous())
            # i_avg, i_std = mat_fourier_avg_std_triton(v_evict_nc_compressed.contiguous(), v_evict_nc.size(1), T=self.T)
            # self.v_scale.append(u_std / i_std)
            # v_evict_nc_compressed = v_evict_nc_compressed * self.v_scale[-1]
            # v_bias_expand = torch.zeros(v_evict_nc_compressed.shape[0], value_states.shape[-1],\
            #     device=v_evict_nc_compressed.device, dtype=torch.float32)
            # v_bias_expand[:, :self.v_non_critical_start[layer_idx]] = u_avg - i_avg * self.v_scale[-1]
            # self.v_bias.append(v_bias_expand)

            v_evict_nc_compressed_expand = torch.zeros(v_evict_nc_compressed.shape[0], v_evict_nc_compressed.shape[1], value_states.shape[-1], \
                device=v_evict_nc_compressed.device, dtype=value_states.dtype)
            v_evict_nc_compressed_expand[:, :, self.v_non_critical_start[layer_idx]:] = v_evict_nc_compressed

            # --- Write new key-values to the cache ---
            k_init = key_states[:, :self.num_init_tokens, :]
            k_local = key_states[:, -self.max_local_len:, :]
            k_il = torch.cat([k_init, k_local], dim=1)
            self.key_cache.append({"k_il":k_il,"k_compressed":k_evict_nc_compressed_expand,"k_uncompressed":k_evict_c.contiguous()})
            v_init = value_states[:, :self.num_init_tokens, :]
            v_local = value_states[:, -self.max_local_len:, :]
            v_il = torch.cat([v_init, v_local], dim=1)
            self.value_cache.append({"v_il":v_il,"v_compressed":v_evict_nc_compressed_expand,"v_uncompressed":v_evict_c.contiguous()})

            return key_states, value_states
        else:
            self._seen_tokens[layer_idx] += 1
            self.max_local_len += 1
            k_il = self.key_cache[layer_idx]["k_il"]
            k_il = torch.cat([k_il,key_states], dim=1)
            self.key_cache[layer_idx]["k_il"] = k_il
            v_il = self.value_cache[layer_idx]["v_il"]
            v_il = torch.cat([v_il,value_states], dim=1)
            self.value_cache[layer_idx]["v_il"] = v_il
        
            return self.key_cache[layer_idx], self.value_cache[layer_idx]

    def to_legacy_cache(self) -> Tuple[Tuple[torch.Tensor], Tuple[torch.Tensor]]:
        raise NotImplementedError("Not implemented to_legacy_cache")

    def __getitem__(self, layer_idx: int) -> List[Tuple[torch.Tensor]]:
        raise NotImplementedError("OptimizedDynamicCache does not support __getitem__")
    ...
    def get_seq_length(self, layer_idx: Optional[int] = 0) -> int:
        ...
        return self._seen_tokens[layer_idx] if layer_idx == 0 else self._seen_tokens[layer_idx] - 1
# ---------------
# ---------------
