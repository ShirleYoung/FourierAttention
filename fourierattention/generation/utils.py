...
class GenerationMixin:
    ...
    def _validate_model_kwargs(self, model_kwargs: Dict[str, Any]):
        ...
        for key, value in model_kwargs.items():
            if value is not None and key not in model_args:
                if key not in ["maxlocallen","maxmidstates","numinittokens","non_critical_dims_path"]:
    ...
    def _prepare_cache_for_generation(
        ...
        # keeps copying the cache thus using much more memory
            ...
            # print("************************************\n")
            # print(f"genration_config:{generation_config._from_model_config}")
            extra_config = ExtraConfig(
                generation_config.max_new_tokens,
                model_kwargs["maxlocallen"],
                model_kwargs["maxmidstates"],
                model_kwargs["numinittokens"],
                model_kwargs["non_critical_dims_path"],
                self.config.max_position_embeddings,
                self.config.num_key_value_heads
            )

            model_kwargs[cache_name] = (
                DynamicCache(extra_config)   # 找self.config里面有LlamaConfig
                if not requires_cross_attention_cache
